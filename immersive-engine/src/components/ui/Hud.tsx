...
/**
 * Interface AMAVYA : marque, libellé du nœud courant, progression, navigation.
 */
...
export default function Hud() {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-10 select-none">
      ...
      {/* libellé du nœud courant (hero / outro surtout) */}
      <div className="absolute inset-x-0 bottom-28 flex flex-col items-center px-8 text-center md:bottom-32">
        <AnimatePresence mode="wait">
          ...
          >
            {node.kicker && (
              <div className="mb-3 text-[11px] uppercase tracking-[0.3em] text-cyan-300">{node.kicker}</div>
            )}
            <h1 className="font-serif text-4xl leading-tight text-white drop-shadow-[0_4px_30px_rgba(0,0,0,0.9)] md:text-6xl">
              {node.title}
            </h1>
      ...
      <div className="absolute bottom-8 left-6 hidden text-[10px] uppercase tracking-[0.3em] text-white/40 md:block">
        molette · flèches · glisser · cliquer une œuvre
      </div>
    </div>
  );
}
